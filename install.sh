...
# Auto-élévation pour exécution en root
if [ "$EUID" -ne 0 ]; then
...
# Initialisation du cluster maître
...
# Créer et appliquer un fichier de surcharge pour activer tous les plugins
log "Applying override manifest for KubeSphere"
...
# Attendre que KubeSphere soit prêt
log "Waiting for KubeSphere to be ready"
kubectl wait --for=condition=ready pods -n kubespere-system --timeout=10m | tee -a "${LOG_FILE}"
